AC.Drawing = {
  ...
  circle: function(start, end, context, type) {
    if (!type) type = 'fill';
    ...
    context[type].call(context);  
    context.closePath();
    return context;
  },
  
  rectangle: function(start, end, context, type) {
    if (!type) type = 'fill';
    context.beginPath();
    context[type + 'Rect'].call(context, start.left, start.top, end.left - start.left, end.top - start.top);
    context.closePath();
    return context;
  }
