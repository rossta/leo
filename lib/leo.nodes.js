...
Leo.PathNode = Leo.Node.extend({
  ...
  load: function(s) {
    ...
    this.segments = _(s.segments).map(function(seg) { return Leo.PathSegment.load(seg); });
  },
  
  lineTo: function(x, y) {
    this.segments.push(new Leo.PathSegment({x: x, y: y, type: "Line"}));
  },
  
  close: function() { this.closed = true; return this; }
});
